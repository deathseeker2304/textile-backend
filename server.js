...
const allowedOrigins = [
    'https://textile-frontend.onrender.com'
];
...
app.post('/api/classes', async (req, res) => {
    ...
    try {
        ...
        if (result.rows.length > 0) {
            res.status(201).json({ message: `Class "${className}" added for Semester ${semester}` });
        } else {
            res.status(409).json({ message: `Class "${className}" already exists for Semester ${semester}` });
        }
    } catch (err) {
...
app.post('/api/teachers', async (req, res) => {
    ...
    try {
        ...
        if (result.rows.length > 0) {
            res.status(201).json({ message: `Teacher "${teacherName}" added` });
        } else {
            res.status(409).json({ message: `Teacher "${teacherName}" already exists` });
        }
    } catch (err) {
...
app.post('/api/pdfs', async (req, res) => {
    ...
    if (!fileName || !url || !semester || !className || !teacherName || !chapterName) {
        return res.status(400).json({ error: 'Missing required PDF fields (fileName, url, semester, className, teacherName, chapterName)' });
    }
    const semesterNum = parseInt(semester, 10);
    if (isNaN(semesterNum)) {
        return res.status(400).json({ error: 'Semester must be a valid number' });
    ...
    try {
        const result = await db.query(
            ...
            [fileName.trim(), url, semesterNum, className.trim(), teacherName.trim(), chapterName.trim()]
        );
...
app.put('/api/pdfs/:id', validateId, async (req, res) => {
...
app.delete('/api/pdfs/:id', validateId, async (req, res) => {
    const pdfId = req.validatedId;
    try {
...
app.put('/api/tasks/:id', validateId, async (req, res) => {
...
app.post('/api/videos', async (req, res) => {
    ...
    if (!videoId || !semester || !className || !teacherName || !chapterName) {
        return res.status(400).json({ error: 'Missing required video fields (videoId, semester, className, teacherName, chapterName)' });
    }
    const semesterNum = parseInt(semester, 10);
    if (isNaN(semesterNum)) {
        return res.status(400).json({ error: 'Semester must be a valid number' });
    ...
    try {
        const result = await db.query(
            ...
            [videoId, semesterNum, className.trim(), teacherName.trim(), chapterName.trim()]
        );
        res.status(201).json(result.rows[0]);
    } catch (err) {
...
app.delete('/api/videos/:id', validateId, async (req, res) => {
...
app.get('/api/notes', async (req, res) => {
    ...
    if (rollNumber) { conditions.push(`roll_number = $${paramIndex++}`); values.push(rollNumber); }
...
app.post('/api/notes', async (req, res) => {
    ...
    if (!content || !semester || !className || !teacherName || !chapterName || !studentName || !rollNumber) {
        return res.status(400).json({ error: 'Missing required note fields (content, semester, className, teacherName, chapterName, studentName, rollNumber)' });
    }
    const semesterNum = parseInt(semester, 10);
    if (isNaN(semesterNum)) {
        return res.status(400).json({ error: 'Semester must be a valid number' });
    ...
    try {
        const result = await db.query(
            ...
            [content, semesterNum, className.trim(), teacherName.trim(), chapterName.trim(), studentName.trim(), rollNumber.trim()]
        );
        res.status(201).json(result.rows[0]);
    } catch (err) {
...
app.delete('/api/notes/:id', validateId, async (req, res) => {
...
app.post('/api/events', async (req, res) => {
    ...
    try {
        const result = await db.query(
            ...
            [title.trim(), event_date, description.trim()]
        );
        res.status(201).json(result.rows[0]);
    } catch (err) {
...
app.delete('/api/events/:id', validateId, async (req, res) => {
...
app.post('/api/exams', async (req, res) => {
    ...
    try {
        const result = await db.query(
            ...
            [exam_date, subject.trim(), exam_time, room.trim()]
        );
        res.status(201).json(result.rows[0]);
    } catch (err) {
...
app.delete('/api/exams/:id', validateId, async (req, res) => {
...
// GET /api/calendarEvents - Fetch calendar events
    ...
    try {
        const result = await db.query(
            `INSERT INTO calendar_events (title, start_time, end_time, all_day, completed)
             VALUES ($1, $2, $3, $4, $5) RETURNING *`,
            [title.trim(), start, end, allDay, completed]
        );
        res.status(201).json(result.rows[0]);
    } catch (err) {
...
app.put('/api/calendarEvents/:id', validateId, async (req, res) => {
    const eventId = req.validatedId;
    ...
    try {
        ...
        const newTitle = title !== undefined ? title.trim() : currentEvent.title;
        const newStart = start !== undefined ? start : currentEvent.start_time;
        const newEnd = end !== undefined ? end : currentEvent.end_time;
...
app.delete('/api/calendarEvents/:id', validateId, async (req, res) => {
    const eventId = req.validatedId;
    try {
...
app.get('/api/health', async (req, res) => {
    try {
        ...
        res.status(500).json({ status: 'DOWN', database: 'disconnected', timestamp: new Date().toISOString() });
    }
});
